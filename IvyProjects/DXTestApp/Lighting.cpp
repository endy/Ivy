...
void DxTestApp::RunLighting()
{
    ...
    LightBuffer light;
    light.pos = Point4(0, 2, 0, 1);
    light.color = Point4(0, 0, 1, 1);

    DxBufferCreateInfo lightBuffer = {0};
    lightBuffer.flags.cpuWriteable = TRUE;
    lightBuffer.elemSizeBytes = sizeof(LightBuffer);
    lightBuffer.pInitialData = &light;
    DxBuffer* pLightBuffer = DxBuffer::Create(pDevice, &lightBuffer);
    ...
    DxShader* pVisStencilPS = DxShader::CreateFromFile(pDevice, "VisStencil", L"DXTestApp/dxtestapp.hlsl");
    ...
    DxShader* pDiffusePS = DxShader::CreateFromFile(pDevice, "Diffuse", L"DXTestApp/dxtestapp.hlsl");

    DxShader* pTransparentPS = DxShader::CreateFromFile(pDevice, "Transparent", L"DXTestApp/dxtestapp.hlsl");
    ...
    ID3D11BlendState* pAlphaBlend = NULL;
    ...
    pDevice->CreateBlendState(&blendDesc, &pAlphaBlend);



    ///@todo Refactor this so we dont have to manipulate camera shader buffer data in main loop
    CameraBufferData* pCameraBufferData = NULL;


    m_pWindow->Show();  ///@todo Refactor this to core class
    ...
    // Set Initital Camera Position
    m_pCamera->Move(Point3(0, 0.7, -5), 0, 17*IvyPi/180.0f);
    ...
    {            
        ...
        LightBuffer* pLight = static_cast<LightBuffer*>(pLightBuffer->Map(pContext));
        memcpy(pLight, &light, sizeof(LightBuffer));
        pLight->pos.x = sin(rotation.x * 10);
        pLight->pos.y = 2;
        pLight->pos.z = cos(rotation.x * 10);
        pLightBuffer->Unmap(pContext);
        ...
        m_pAppConstBuffer->BindPS(pContext, 2);
        pLightBuffer->BindPS(pContext, 0);
        ...
        pCameraBufferData->worldMatrix      = XMMatrixScaling(0.1, 0.1, 0.1) * XMMatrixTranslation(pLight->pos.x, pLight->pos.y, pLight->pos.z); 
        ...
        pDiffusePS->Bind(pContext);

        pBunnyMesh->Bind(pContext);
        pBunnyMesh->DrawInstanced(pContext, 25);

        pCameraBufferData = reinterpret_cast<CameraBufferData*>(pCameraBuffer->Map(pContext));
        pCameraBufferData->worldMatrix      = XMMatrixTranslation(0, -2, 0) * XMMatrixScaling(10, 1, 10);
        pCameraBufferData->viewMatrix       = m_pCamera->W2C();
        pCameraBufferData->projectionMatrix = m_pCamera->C2S(); 
        pCameraBuffer->Unmap(pContext);
        ...
        // Light Projection
        pCameraBufferData = reinterpret_cast<CameraBufferData*>(pCameraBuffer->Map(pContext));
        pCameraBufferData->worldMatrix      = XMMatrixScaling(0.1, 0.1, 0.1) * XMMatrixTranslation(0, 0, 0); 
        pCameraBufferData->viewMatrix       = m_pCamera->W2C();
        pCameraBufferData->projectionMatrix = m_pCamera->C2S(); 
        pCameraBuffer->Unmap(pContext);

        // Get blend state to restore
        ID3D11BlendState* pBackupBlendState = NULL;
        FLOAT backupBlendFactor[4];
        UINT backupSampleMask = 0;
        pContext->OMGetBlendState(&pBackupBlendState, &backupBlendFactor[0], &backupSampleMask);

        // Set earlier blend state
        pContext->OMSetBlendState(pAlphaBlend, &backupBlendFactor[0], backupSampleMask);

        pPosTexVS->Bind(pContext);
        pTransparentPS->Bind(pContext);
        pCubeMesh->Bind(pContext);
        pCubeMesh->Draw(pContext);

        // Set earlier blend state
        pContext->OMSetBlendState(pBackupBlendState, &backupBlendFactor[0], backupSampleMask);
        ...
        // VIZ DEPTH
        vizDepth.TopLeftX = m_screenWidth - (m_screenWidth / 4);
        vizDepth.TopLeftY = (m_screenHeight / 2);
        pContext->RSSetViewports(1, &vizDepth);
